...
Uso:
    python -m src.services.receita_federal <arquivo_prospeccao>

Exemplo:
    python -m src.services.receita_federal "data/input/Saude - RIB_15-18-42_04-03-2026.xlsx"
"""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return {
                ...
                "cnae_principal": dados.get("atividade_principal", [{}])[0].get(
                    "text", ""
                ),
...
def validar_cnpjs_lote(caminho_arquivo: str, limite: int = None):
    ...
    for idx, row in df.iterrows():
        ...
        if resultado.get("sucesso"):
            ...
            erros += 1
            print(
                f"[{idx+1}/{total}] {nome:<30} → ERRO: {resultado.get('erro', 'N/A')[:20]}"
            )
    ...
    # Garante que a coluna situação existe e trata nulos
    if "situacao" in df_resultados.columns:
        df_resultados["situacao"] = df_resultados["situacao"].fillna("").astype(str)
    else:
        df_resultados["situacao"] = ""

    # Identifica divergências (empresa encontrada mas não está ativa)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Uso: python -m src.services.receita_federal <arquivo_prospeccao> [limite]")
        print("")
        print("Exemplos:")
        print('  python -m src.services.receita_federal "data/input/Saude - RIB.xlsx"')
        print('  python -m src.services.receita_federal "data/input/Saude - RIB.xlsx" 10')
        print("")
        print("APIs: BrasilAPI (primária) + ReceitaWS (fallback)")
        print("Custo: GRATUITO")
        sys.exit(1)
